...
class _TestABPageState extends State {
  ...
  ///构建一个列表 ListView
  buildListView() {
    myPrint("buildListView ...");

    ///懒加载模式构建
    return ListView.builder(
      ///子Item的构建器
      itemBuilder: (BuildContext context, int index) {
        myPrint("itemBuilder ...index = $index");
...
class TestBean {
  ...
  TestBean({required this.name, required this.isCollect});
}
