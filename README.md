...
主isolate: Dart,单线程事件循环

要想自己实现多线程使用isolate, 注意不共享内存，通过消息直接传递消息


不过如果你的诉求是希望能够实现动态界面，目前业界已经有在上层封装DSL实现Flutter动态布局的实践了，可以看看GMTC大会上美团和微信的分享


一级页面和二级页面都是通过provide进行状态管理的
有一些通用的改造方法，比如不要直接在根部直接套StatefulWidget，而是把可变部分最小化；引入key，或是重写canUpdate方法

可变化的部分，做到最小化组件

我们的App大多数场景下都是静默状态，单线程对大多数场景已经足够用了：
1.I/O、网络请求操作系统提供了异步的API
2.CPU密集型的操作用Isolate即可

业界已经有团队使用JSCore实现动态布局了，预计下半年会有一些较为成熟的方案出现


webview 加载网页的过程
接下来，我以加载过程为例，和你说明这个过程的复杂性。浏览器控件加载 HTML5 页面的 HTML 主文档；加载过程中遇到外部 CSS 文件，
浏览器另外发出一个请求，来获取 CSS 文件；遇到图片资源，浏览器也会另外发出一个请求，来获取图片资源。这是异步请求，
并不会影响 HTML 文档的加载。加载过程中遇到 JavaScript 文件，由于 JavaScript 代码可能会修改 DOM 树，
因此 HTML 文档会挂起渲染（加载解析渲染同步）的线程，直到 JavaScript 文件加载解析并执行完毕，才可以恢复 HTML 文档的渲染线程。
JavaScript 代码中有用到 CSS 文件中的属性样式，于是阻塞，等待 CSS 加载完毕才能恢复执行。


dart 中的class 即可以被子类继承extends(可以使用所有)，也可以被implement(需要复写所有成员，只是继承了一个空盒子)
扩展父类，单继承， with



widget     element(widget 引用， RenderObject 引用)     RenderObject  三颗树


多个Widget（销毁前后）会对应一个Element， 场景是一个widget的颜色变化，widget会重新创建,但是element不会
